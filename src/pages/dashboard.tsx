...
import { RouteListPage as ShowCaseList } from 'pages/routes';

function DashboardPage() {
  const { session } = useSession();

  const routeStat = useSWR<PaginatedInterface<RouteInterface>>(
    () => `/routes`,
    () => getRoutes({ limit: 1, offset: 0 }),
  );
  ...
  const bookingStat = useSWR<PaginatedInterface<BookingInterface>>(
    () => `/bookings`,
    () => getBookings({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Routes',
      result: routeStat?.data?.totalCount,
      isLoading: routeStat.isLoading,
    ...
    {
      title: 'Bookings',
      result: bookingStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`routes`} passHref legacyBehavior>
                  <Button
